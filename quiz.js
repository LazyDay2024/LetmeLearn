    ...
    const choicesHtml = item.choices.map((choice, choiceIndex) => {
      ...
      return `
        <label class="choice-item" for="${choiceId}">
          <input type="radio" id="${choiceId}" name="question_${index}" value="${safeChoice}">
...
function gradeQuiz() {
  ...
  questions.forEach((item, index) => {
    ...
    // Helper to normalize strings: remove ALL spaces, convert to lowercase
    ...
    const isMatch = (val1, val2) => {
      ...
      // Remove leading prefix like 'ก.', 'A.', '1)', 'ข้อ 1' and compare again
      const stripPrefix = (s) => s.replace(/^[ก-ฮa-z0-9][\.\)]/, '').replace(/^ข้อ[0-9]+/, '');
      if (stripPrefix(n1) === stripPrefix(n2)) return true;

      return false; // Avoid includes() to prevent "1" matching "10" bug
    };

    const correctAnswerStr = String(item.answer);

    // Revert all choice-item colors and highlight the correct one
    labels.forEach(radio => {
      radio.closest('.choice-item').classList.remove('correct-choice', 'incorrect-choice');
      
      if (isMatch(radio.value, item.answer)) {
         radio.closest('.choice-item').classList.add('correct-choice-highlight');
      }
    });

    if (selected) {
      const isCorrect = isMatch(selected.value, item.answer);
      ...
      if (isCorrect) {
        ...
        resultBox.textContent = `ผิด ✗ คำตอบที่ถูกคือ: ${correctAnswerStr}`;
      }
    } else {
      resultBox.className = "answer-result incorrect";
      resultBox.textContent = `ยังไม่ได้เลือกคำตอบ | คำตอบที่ถูกคือ: ${correctAnswerStr}`;
    }
  });
